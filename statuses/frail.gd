class_name FrailStatus
extends Status

const REDUCTION := -0.25

func get_tooltip() -> String:
	return tooltip % duration

func initialize_status(target: Node) -> void:
	assert(target.get("modifier_handler"), "No modifiers on %s" % target)

	var block_modifier: Modifier = target.modifier_handler.get_modifier(Modifier.Type.BLOCK_GAINED)
	assert(block_modifier, "No block modifier on %s" % target)

	var frail_value := block_modifier.get_value("frail")
	if not frail_value:
		frail_value = ModifierValue.create_new_modifier("frail", ModifierValue.Type.PERCENT_BASED)
		block_modifier.add_new_value(frail_value)

	frail_value.percent_value = REDUCTION

	if not status_changed.is_connected(_on_status_changed):
		status_changed.connect(_on_status_changed.bind(block_modifier))

func _on_status_changed(block_modifier: Modifier) -> void:
	if duration <= 0:
		block_modifier.remove_value("frail")
